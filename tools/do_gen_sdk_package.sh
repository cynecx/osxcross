...
for SDK in $SDKS; do
  TMP=$(mktemp -d "$CURRENT_DIRECTORY/tmp-xcodeapp-XXXXXXXXXXXXXX")

  echo -n "packaging $(echo "$SDK" | sed -E "s/(.sdk|.pkg)//g") SDK "
  echo "(this may take several minutes) ..."

  if [[ $SDK == *.pkg ]]; then
    cp $(realpath $SDK) $WDIR
    continue
  fi
  cp -r $(realpath $SDK) $TMP/$SDK &>/dev/null || true
  ...
  pushd $TMP &>/dev/null
  $TAR -cf - * | $COMPRESSOR -9 -c - > "$CURRENT_DIRECTORY/$SDK.$PKGEXT"
  popd &>/dev/null
